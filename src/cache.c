...
struct cache_entry *alloc_entry(char *path, char *content_type, void *content,
    ...
    if (new_entry->path == NULL || new_entry->content_type == NULL) {
        free(new_entry->path);
...
void cache_put(struct cache *cache, char *path, char *content_type,
    ...
    struct cache_entry *existing_entry = hashtable_get(cache->index, path);

    if (existing_entry != NULL) {
        dllist_move_to_head(cache, existing_entry);
        return;
    }
